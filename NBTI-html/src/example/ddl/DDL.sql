...
CREATE TABLE `problem` (
                           ...
                           `category_id`       INT NOT NULL COMMENT '분야 ID',
                           `answer_type_id`    INT NOT NULL COMMENT '답안 유형 ID',
                           `content_image_url` VARCHAR(2048) NOT NULL COMMENT '본문 이미지 URL',
                           `correct_answer`    VARCHAR(255) NOT NULL COMMENT '정답',
                           `is_deleted`        ENUM('Y','N') NOT NULL DEFAULT 'N' COMMENT '삭제 여부',
                           `level`             INT NOT NULL DEFAULT 1 COMMENT '난이도',
...
CREATE TABLE `objection` (
                             ...
                             `rejected_reason` VARCHAR(255) NULL COMMENT '거절 사유',
